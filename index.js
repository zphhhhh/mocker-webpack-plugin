...
class EasymockWebpackPlugin {
    /**
     * generete options
     * @param {EasymockOptions} options
     */
    constructor(options) {
    ...
    apply(compiler) {
        compiler.hooks.afterPlugins.tap('EasymockWebpackPlugin', compiler => {
            const { context, devServer } = compiler.options;

            if (!devServer) {
                throw Error('The easymock-webpack-plugin could run only with WebpackDevServer!');
            }
